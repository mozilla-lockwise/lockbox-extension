...
async function fetchFromFxA(reason, url, config) {
...
async function fetchToken(code, challenge) {
  ...
  let response = await fetchFromFxA("OAUTH token", `${OAUTH_URL_PREFIX}/token`, {
    method: "post",
    headers: {
...
async function fetchProfile(token) {
  let response = await fetchFromFxA("profile", `${OAUTH_PROFILE_URI}/profile`, {
    method: "get",
    headers: {
      "Accept": "application/json",
